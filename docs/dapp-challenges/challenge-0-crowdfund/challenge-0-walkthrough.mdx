...
The `useEffect` hook updates the UI based on the contract values. This hook also checks if the target amount has been reached by comparing it with the token balance.
...
The component subscribes to the 'pledged_amount_changed' and 'target_reached' events using the `sorobanEventsContext.subscribe` function. When these events occur, the UI state is updated accordingly.
...
Now that we've explored the code for our crowdfunding dapp, lets take on the [Crowdfund Dapp Challenge!](../challenge-0-crowdfund)
